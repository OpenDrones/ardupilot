...
void Copter::do_aux_switch_function(int8_t ch_function, uint8_t ch_flag)
{
    ...
    switch(ch_function) {
        ...
        case AUXSW_LAND:
            if (ch_flag == AUX_SWITCH_HIGH) {
                if(set_mode(LAND, MODE_REASON_TX_COMMAND)) {
                    // alert user to mode change
                    AP_Notify::events.user_mode_change = 1;
                } else {
                    // alert user to mode change failure
	   ...
	   case AUXSW_RELAY4:
        ...
        case AUXSW_BRAKE:
            // brake flight mode
            if (ch_flag == AUX_SWITCH_HIGH) {
                if(set_mode(BRAKE, MODE_REASON_TX_COMMAND)) {
                    // alert user to mode change
                    AP_Notify::events.user_mode_change = 1;
                } else {
					// alert user to mode change failure
        ...
        case AUXSW_THROW:
            // throw flight mode
            if (ch_flag == AUX_SWITCH_HIGH) {
                if (set_mode(THROW, MODE_REASON_TX_COMMAND)) {
                    // alert user to mode change
                    AP_Notify::events.user_mode_change = 1;
				} else {
					// alert user to mode change failure
        ...
        case AUXSW_CRUISE:
            // cruise flight mode
            if (ch_flag == AUX_SWITCH_HIGH) {
            	if (set_mode(CRUISE, MODE_REASON_TX_COMMAND)) {
					// alert user to mode change
                    AP_Notify::events.user_mode_change = 1;
				} else {
					// alert user to mode change failure
        ...
        case AUXSW_WPCRUISE:
            ...
            if (ch_flag == AUX_SWITCH_HIGH) {
                   if(set_mode(WPCRUISE, MODE_REASON_TX_COMMAND)) {
                    // alert user to mode change
                    AP_Notify::events.user_mode_change = 1;
                } else {
                    // alert user to mode change failure
