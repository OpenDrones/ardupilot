...
void AC_PosControl::relax_alt_hold_controllers(float throttle_setting)
{
	...
	_pid_rate_z.set_integrator((throttle_setting-_motors.get_throttle_hover())*1000.0f);
}
...
void AC_PosControl::init_takeoff()
{
	...
	_pid_rate_z.set_integrator((_motors.get_throttle()-_motors.get_throttle_hover())*1000.0f);
}
...
void AC_PosControl::rate_to_accel_z()
{
    ...
    if (ratio >= POSCONTROL_RATIO_THRESHOLD && ratio <= (1 - POSCONTROL_RATIO_THRESHOLD)) {
        _last_imu_ok_time_ms = AP_HAL::millis();
    } else if (AP_HAL::millis() - _last_imu_ok_time_ms > POSCONTROL_IMU_ERROR_TIMEOUT_MS) {
