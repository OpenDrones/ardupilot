...
bool AP_Mission::calc_simple_grid()
{
    ...
    for (uint8_t i = 1; i < num_commands(); i++)
    {
        ...
        polygon[i-1].y = (cmd_polygon.content.location.lat - reference_location.lat) * LOCATION_SCALING_FACTOR_CM;
        polygon[i-1].x = (cmd_polygon.content.location.lng - reference_location.lng) * LOCATION_SCALING_FACTOR_CM * long_scale;
    }
    ...
    int32_t diagdist = pythagorous2(xmax - xmin, ymax - ymin);
    ...
    {
        waypoint_cmd.content.location.lat = reference_location.lat + int (waypoint[m].y * LOCATION_SCALING_FACTOR_CM_INV);
        waypoint_cmd.content.location.lng = reference_location.lng + int (waypoint[m].x * LOCATION_SCALING_FACTOR_CM_INV / long_scale);
        waypoint_cmd.content.location.alt = 300;
